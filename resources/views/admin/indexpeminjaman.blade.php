...
<section id="list">
  ...
  <div class="container">
    ...
    <form action="{{ route('peminjam.indexpeminjaman') }}" method="GET">
      @if(session('success'))
      ...
      <table class="table-striped table">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>
              @if (($tayo->Status) == 1)
              <a class="btn btn-warning" href="{{ route('peminjam.updatepeminjaman', ['id' => $tayo->id]) }}" role="button">Waiting</a>
              @elseif (($tayo->Status) == 2)
              <a class="btn btn-success" href="{{ route('peminjam.updatepeminjaman', ['id' => $tayo->id]) }}" role="button">Approve</a>
              @elseif (($tayo->Status) == 3)
              <a class="btn btn-danger" href="{{ route('peminjam.updatepeminjaman', ['id' => $tayo->id]) }}" role="button">Reject</a>
              @else
